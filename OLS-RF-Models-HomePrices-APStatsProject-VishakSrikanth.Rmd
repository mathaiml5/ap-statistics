...
```{r, handle_missing_home_features}
...
```{r, consolidate_categorical_vars}
# Deck/Porch
...
```{r, handle_ordinal_vars, results='hide'}
...
```{r, exploratory_analysis, warning=FALSE, error=FALSE}
...
```{r, correlation_plot, warning=FALSE, error=FALSE}
...
```{r full_regression, results='hide' }
...
```{r reduced_fit, results='hide'}
summary(fit.reduced)
```


```{r, reduced_model, results='hide',fig.height=4, fig.width=6}
par(mfrow=c(2,2))
plot(fit.reduced)
```
